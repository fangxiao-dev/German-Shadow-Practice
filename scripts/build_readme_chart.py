...
def build_svg(
    ...
    e(
        f'  <rect x="{PAD}" y="{PAD}" width="4" height="48" '
        f'rx="2" fill="{ORANGE_HI}"/>'
    )
    e(
        f'  <text x="{PAD + 14}" y="36" font-family="{FONT}" '
        f'font-size="18" font-weight="650" fill="{TEXT_PRI}">German study</text>'
    )
    e(
        f'  <text x="{PAD + 14}" y="53" font-family="{FONT}" '
        f'font-size="10" fill="{TEXT_SEC}">12-week shadow practice</text>'
    )
    e(
        f'  <line x1="{PANEL_DIVIDER_X}" y1="24" x2="{PANEL_DIVIDER_X}" '
    ...
    for value, label, color, y in stat_rows:
        e(
            f'  <text x="{PAD}" y="{y}" font-family="{FONT}" '
            f'font-size="22" font-weight="700" fill="{color}">{value}</text>'
        )
        e(
            f'  <text x="{PAD + 66}" y="{y - 8}" font-family="{FONT}" '
            f'font-size="10" font-weight="600" fill="{TEXT_PRI}">{label}</text>'
        )
        e(
            f'  <line x1="{PAD}" y1="{y + 10}" x2="{PANEL_DIVIDER_X - 22}" '
