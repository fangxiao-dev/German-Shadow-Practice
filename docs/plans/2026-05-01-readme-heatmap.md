...
TOTAL_W = 560
TOTAL_H = 220
GRID_X = TOTAL_W - PAD - GRID_W
GRID_Y = 68
MONTH_Y = 53
DAY_LABEL_X = GRID_X - DAY_LABEL_W
PANEL_DIVIDER_X = DAY_LABEL_X - 34
...
def build_svg(
    ...
    # ── Left metric rail ──
    e(f'  <rect x="{PAD}" y="{PAD}" width="4" height="48" '
      f'rx="2" fill="{ORANGE_HI}"/>')
    e(f'  <text x="{PAD + 14}" y="36" font-family="{FONT}" '
      f'font-size="18" font-weight="650" fill="{TEXT_PRI}">German study</text>')
    e(f'  <text x="{PAD + 14}" y="53" font-family="{FONT}" '
      f'font-size="10" fill="{TEXT_SEC}">12-week shadow practice</text>')
    e(f'  <line x1="{PANEL_DIVIDER_X}" y1="24" x2="{PANEL_DIVIDER_X}" '
      f'y2="{TOTAL_H - 24}" stroke="{BORDER}" stroke-width="1"/>')

    stat_rows = [
        (str(study_days), "study days", ORANGE_HI, 88),
        (str(total_words), "words committed", TEXT_PRI, 124),
        (str(avg), "avg per session", TEXT_PRI, 160),
    ]
    for val, label, val_color, y in stat_rows:
        e(f'  <text x="{PAD}" y="{y}" font-family="{FONT}" '
          f'font-size="22" font-weight="700" fill="{val_color}">{val}</text>')
        e(f'  <text x="{PAD + 66}" y="{y - 8}" font-family="{FONT}" '
          f'font-size="10" font-weight="600" fill="{TEXT_PRI}">{label}</text>')
        e(f'  <line x1="{PAD}" y1="{y + 10}" x2="{PANEL_DIVIDER_X - 22}" '
          f'y2="{y + 10}" stroke="{TEXT_MUTED}" stroke-width="1" opacity="0.45"/>')
